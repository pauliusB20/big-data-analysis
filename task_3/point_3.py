...
def process_mmsi_chunk(mmsi_chunk: list[str]) -> tuple[int, int]:
    ...
    query = {
        "mmsi": {"$in": mmsi_chunk,}, 
        "vessel_type": {"$in": ["Class A", "Class B"]}, # No base stations, etc.
        "latitude": {"$exists": True, "$ne": None,
                     "$gte": 53.0, "$lte": 66.0},   # Baltic Sea Box    
        "longitude": {"$exists": True, "$ne": None,
                      "$gte": 10.0, "$lte": 30.0}, # Baltic Sea Box    
        "sog": {"$exists": True,
                "$gte": 0, "$nin": [102.3, None]},        # No negative speed, no 102.3 error code, up to 102.3       
        "cog": {"$exists": True, "$ne": None,
                "$gte": 0, "$lt": 360.0},           # Valid compass degrees, up to 360    
        "heading": {"$exists": True,
                    "$gte": 0, "$lt": 360, "$nin": [511, None]}, # No 511 error code
        "rot": {"$exists": True, "$nin": [-128, None]},        # No -128 error code                    
